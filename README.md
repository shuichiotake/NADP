...
For each word, the file gives the norms from the orher words by the ascending order. The second main1-file creates the embedding data for using 'SentEval' (see https://github.com/facebookresearch/SentEval) in ~~/Data_DS/Models. The created file of the data consists of 73404 lines, and the line has one word at the top and 300 real numbers after that, separeted by the bland. Temporal data for embeddings with noise are created in ~~/Data_DS/1_VocKpebd. 
