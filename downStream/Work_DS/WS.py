...
class WS:
    ...
    def __init__(self, ut):
        ...
        self._dir_Data1  = "../Data_DS/1_VocKpebd/" 
        self._dir_Mdl  = "../Data_DS/Models/" 
    ...
    def m1_create_NG(self, wrdKebds, jacs, epss, lbls, onT):
        ...
        for lbl,knd,jac,eps in itertools.product(lbls,knds,jacs,epss):
            ...
            for cnt0,(wrd,ebd) in enumerate(sorted(wrdKebds.items())):
                # get noi  
                noi = self._ut.analytic_gaussian(eps,alpha_mod,float(jac)) if knd == "AG" else self._ut.analytic_gaussian_var(wrd,eps,alpha_mod,voc_Delta)
                # get pbd
            ...
            mdl = "Noi_" + knd + "/"
            fle = self._dir_Data1 + mdl + "wrdKpbds_" + tal + ".pkl"
            if onT: fle += "_onT"
            with open(fle,"wb") as f:              pickle.dump(wrdKpbds,f)
    #:
    def m1_create_J(self, wrdKebds, jacs, epss, lbls, wrdKnrms4S, onT):
        knd = "AJ"
        # ----- init
        topK = 10
        with open(self._dir_common+"alpha_mod.pkl","rb") as f:           alpha_mod = pickle.load(f)
        ...
        for lbl,jac,eps in itertools.product(lbls,jacs,epss):
            ...
            if onT: fle += "_onT"
            with open(fle,"wb") as f:              pickle.dump(wrdKpbds,f)
            #DD print("EE: fle,len(wrdKpbds) =",fle,len(wrdKpbds),"\n")
    #:
    def m1_create_L(self, wrdKebds, epss, lbls, onT):
        knd = "AL"
        # -----
        for eps,lbl in itertools.product(epss,lbls):
            ...
            with open(fle,"wb") as f:              pickle.dump(wrdKpbds,f)
    #:
    def m1_create_M(self, wrdKebds, epss, lbls, onT):
        knd = "M"
        # ----- pre
    ...
    def m2_create_NGJL(self, jacs, epss, lbls, onT):
        knds = ["AGv","AG","AJ","AL","M"]     # AG->NADP, AG->Gaussian
        for lbl,knd,jac,eps in itertools.product(lbls,knds,jacs,epss):
            #DD print("@@: onT,knd,jac,eps,lbl =", onT,knd,jac,eps,lbl)
            # - load
            ...
            tal = "_".join([str(knd),str(jac),str(eps),str(lbl)])
            mdl = "Noi_" + knd + "/"
            fle = self._dir_Mdl + mdl + "model_" + tal + ".txt"
