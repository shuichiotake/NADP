...
class UT(UTRoot):
    ...
    """                                                                                                             jaccard preserving noise で使用する関数の定義                                                                                                                                                                                   引数は, 1.単語, 2.jaccard index, 3.自然数k(top_kの近傍を考える場合), 4.単語が属する単語集合に対し, top_lのコサイン類似度の値を辞書化したもの(l >= k, キーが単語, 値はtop_lのコサイン類似度の値を降順にリスト化したもの)で, 単語にnoiseを加えたとき, ジャッカード係数が引数で入力した値になると予想される標準偏差sdを返す
　　                                                                                                                """
    ...
    """
    jaccard preserving noise func. 2-part ver.(sim) の定義
    
    引数は, 1.単語, 2.jaccard index, 3.自然数k(top_kの近傍を考える場合), 4.単語が属する単語集合に対し, top_lのコサイン類似度の値を辞書化したもの(l >= k, キーが単語, 値はtop_lのコサイン類似度の値を降順にリスト化したもの)で, 単語にnoiseを加えたとき, ジャッカード係数が引数で入力した値になると予想される標準偏差sdを返す
    ...
    """
    jaccard preserving noise func. 2-part ver.(dist) の定義
    
    引数は, 1.単語, 2.jaccard index, 3.自然数k(top_kの近傍を考える場合), 4.単語が属する単語集合に対し, top_lのユークリッド距離の値を辞書化したもの(l >= k, キーが単語, 値はtop_lのユークリッド距離の値を昇順にリスト化したもの)で, 単語にnoiseを加えたとき, ジャッカード係数が引数で入力した値になると予想される標準偏差sdを返す 
　　
　　"""
...
　　"""
    ...
    # -- analytic_laplace_noise?; added just in case
